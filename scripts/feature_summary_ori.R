...
setwd('/Users/pollet/home1/PolyploidiX/Li_Zhen/Retrozyme_project/Retrozyme_bioinfo_Zhen/git_repository/retrozyme/')


## To obtain summary information
Retrozyme_summary=function(species){
  print(species)
  retrozyme_tblfile=paste('retrozyme_data/Test_Retrozymes_detection/', species, '/repeat_summary.tbl', sep = '')
  ...
  for(clust in active_family){
    clusterfile=paste('retrozyme_data/Test_Retrozymes_detection/', species,'/Clusters/', clust, '.clust.bed6', sep = '')
  ...
  MONOMER_PROPORTION=median(monomer_proportion_list)   ## Use median instad of mean to represent the monomer proportion.
  return(c(species, ACTIVE_COUNT, RPT_LIMIT, FAMILY_NUM, MONOMER_PROPORTION))
}

Retrozyme_summary_df=as.data.frame(do.call('rbind', lapply(list.files('retrozyme_data/Test_Retrozymes_detection/'), Retrozyme_summary)), stringsAsFactors = F)
...
Retrozyme_VARY=function(species){
  ...
  for(family in active_family){
    clusterfile=paste('retrozyme_data/Test_Retrozymes_detection/', species,'/Clusters/', family, '.clust.bed6', sep = '')
    cluster_bed = read.csv2(clusterfile, header = F, sep = '\t', stringsAsFactors = F)
    colnames(cluster_bed)=c('chrmid', 'start', 'end', 'identity', 'coverage', 'strand', 'cluster_id')
    cluster_idlist=cluster_bed$cluster_id
    cluster_iddf=as.data.frame(table(cluster_idlist), stringsAsFactors = F)
...
Retrozyme_VARY_df=as.data.frame(do.call('rbind', lapply(list.files('retrozyme_data/Test_Retrozymes_detection/'), Retrozyme_VARY)), stringsAsFactors = F)
