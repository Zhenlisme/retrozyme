...
Retrozyme_rnafold=function(species){
  ...
  rnafold_df=as.data.frame(do.call('rbind' , lapply(rtzname_list, function(x){
    rnafold_file=paste(retrozyme_detection_folder_species,'/HMM_cluster/', x, '.hmm.clust', sep = '')
  ...
  repeat_summary_file=here(retrozyme_detection_folder_species, "repeat_summary.tbl")
...
All_rnafold_df=as.data.frame(do.call('rbind', lapply(list.files(retrozyme_detection_folder), Retrozyme_rnafold)), 
              stringsAsFactors = F)
...
rnafold_distribution_image=here("img","rnafold_distribution.png")
ggsave(rnafold_distribution_image, width = 12, height = 6)

ggplot(All_rnafold_df, aes(x=rnafold))+
  geom_histogram(binwidth = 0.5,color='black', fill='white')

rnafold_distribution_sum_image = here("img","rnafold_distribution_sum.png")
ggsave(rnafold_distribution_sum_image, width = 6, height = 6)
...
rnafold_appearence_image = here("img","rnafold_appearence.png")
ggsave(rnafold_appearence_image, width = 6, height = 6)
