...
class Structure_analyze:
    ...
    def FullCopyDetect(self, query_file):
        clusternum_dict={}
        blastn_tbl='blastn.tbl'
        run_blastn = subprocess.Popen(
            ['blastn', '-db', self.genomdb, '-query', query_file, '-num_threads', str(Args.process), '-outfmt',
             '6 qseqid sseqid pident length mismatch qstart qend sstart send evalue qlen', '-out', blastn_tbl])
        run_blastn.wait()
        sys.stdout.write('Finish of homology searching of retrozyme in genome.\n')
        bed6_line_dict = defaultdict(list)
        with open(blastn_tbl, 'r') as F:
            for line in F:
                splitlines=line.rstrip().split('\t')
                identity = float(splitlines[2])
                coverage = (int(splitlines[6])-int(splitlines[5])+1)/int(splitlines[-1])
                if coverage>=0.8 and identity >= 80:
                    subject_start, subject_end=splitlines[7:9]
                    strand='+'
                    if int(splitlines[7])>int(splitlines[8]):
                        subject_start=splitlines[8]
                        subject_end=splitlines[7]
                        strand='-'
                    bed6_line_dict[splitlines[0]].append([splitlines[1], subject_start, subject_end, splitlines[0], str(identity), strand])

        for query in bed6_line_dict:
            bed_file = ''.join([query, '.bed6'])
            bed_lines = sorted(bed6_line_dict[query], key=lambda x: [x[0], int(x[1])])
            with open(bed_file, 'w') as F:
                for line in bed_lines:
                    F.write('\t'.join(line))
                    F.write('\n')

            #### To cluster different full hits by distance (1000 bp)
            full_copy_cluster='full_copy_clust'
            if not os.path.exists(full_copy_cluster):
                os.mkdir(full_copy_cluster)
            cluster_file = ''.join([full_copy_cluster,'/',query, '.clust'])
            with open(cluster_file, 'w') as F:
                cluster_bedtools = subprocess.Popen(['bedtools', 'cluster','-i', bed_file, '-s', '-d', '1000'],
                                                    stderr=subprocess.DEVNULL, stdout=F)
                cluster_bedtools.wait()

            with open(cluster_file, 'r') as F:
                num_clusters = len({line.rstrip().split('\t')[-1] for line in F})
            clusternum_dict[query] = num_clusters
            os.remove(bed_file)

        return clusternum_dict
    ...
    def run_process(self):
        retrozyme_dict={}   ## save the information of retrozyme candidate
        ...
        for result in run_result:
            ...
            for subloc in result_get:
                retrozyme_dict[subloc[3]]=subloc
                if subloc[4] == 'trf':
                    seq=self.getseq([subloc[0], subloc[1], int(subloc[1])+int(subloc[5])], subloc[-1])   ## Only extract the first unit of tandem repeats
                    seq_list.append(''.join(['>', subloc[3], '\n', seq, '\n']))
                else:
                    seq=self.getseq(subloc[:3], subloc[-1])   ### extract the full unit of ltr retrozyme
                    seq_list.append(''.join(['>', subloc[3], '\n', seq, '\n']))

        if seq_list:
            retrozyme_candidate_file='first_unit.fa'
            with open(retrozyme_candidate_file,'w') as F:
                F.writelines(seq_list)
            cluster_num=self.FullCopyDetect(retrozyme_candidate_file)
            with open('retrozyme.tbl','w') as F:
                F.write('\t'.join(['Chrid', 'start', 'end', 'name', 'type', 'unit_length/insertion_length', 'repeats', 'strand', 'cluster_num_1000\n']))
                for query in retrozyme_dict:
                    opline=retrozyme_dict[query]
                    c_num=str(cluster_num[query])
                    opline.append(c_num)
                    F.write('\t'.join(opline))
                    F.write('\n')
        else:
            sys.stdout.write('No retrozyme detected!\n')
...
def sub_retro(Args):
    ...
    SA = Structure_analyze(genome=GENOME, hhmtbl='hhm-block.tbl', genome_blastn_db=BLASTNDB)
    SA.run_process()
...
if __name__ == "__main__":
    ...
    parser_main.add_argument('-gdb', '--blastndb', type=str, required=False, default='',
    ...
    parser_retrozyme.add_argument('-gdb','--blastndb', type=str, required=False, default='', help='Blastn database name')
